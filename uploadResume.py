...
# Load resumes from MySQL using pandas with ID range filter
query = f"""
SELECT CAST(id AS CHAR) AS R_id, content, name, s3
FROM resume 
WHERE content IS NOT NULL AND name IS NOT NULL 
...
if len(resume_filtered) > 0:
    for _, row in resume_filtered.iterrows():
        ...
        # Construct the bucket URL (using the write_file function which returns the correct URL)
        resume_url = file_path  # The URL returned by write_file is already correctly formatted.
        ...
        try:
            ...
            # Check contents of temp_table for debugging
            with engine.connect() as conn:
                result = conn.execute(text("SELECT * FROM temp_table"))
                for r in result:
                    print(f"Temp Table Row: {r}")
